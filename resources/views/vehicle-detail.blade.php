...
<div class="container space-y-4 py-6">
    ...
    <div class="grid gap-8 lg:grid-cols-3">
        ...
        <div class="space-y-4 lg:col-span-2">
            ...
            @php
                $videoEmbed = null;
                if (($vehicle->video_provider ?? null) === 'youtube' || str_contains($vehicle->video_url, 'youtube') || str_contains($vehicle->video_url, 'youtu.be')) {
                    preg_match('/(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([a-zA-Z0-9_-]+)/', $vehicle->video_url, $yt);
                    $videoEmbed = isset($yt[1]) ? 'https://www.youtube.com/embed/'.$yt[1] : null;
                } elseif (($vehicle->video_provider ?? null) === 'vimeo' || str_contains($vehicle->video_url, 'vimeo.com')) {
                    preg_match('/vimeo\.com\/(\d+)/', $vehicle->video_url, $vm);
                    $videoEmbed = isset($vm[1]) ? 'https://player.vimeo.com/video/'.$vm[1] : null;
                }
            @endphp
            <div class="detail-section">
                <h2 class="text-foreground mb-4 text-lg font-semibold">{{ __('messages.pages.vehicles.detail.video') }}</h2>
                @if($videoEmbed)
                <div class="aspect-video overflow-hidden rounded-lg border border-border">
                    <iframe
                        src="{{ $videoEmbed }}"
                        class="h-full w-full"
                        frameborder="0"
                        ...
                        allowfullscreen
                        title="{{ __('messages.pages.vehicles.detail.video') }}"
                    ></iframe>
                </div>
                @else
                <a href="{{ $vehicle->video_url }}" target="_blank" rel="noopener noreferrer" class="text-primary hover:underline">
                    {{ __('messages.pages.vehicles.detail.watch_video') }}
                </a>
                @endif
            </div>
            @endif
        ...
        <div class="detail-section bg-gray-50">
            ...
            <div class="border-border mt-6 border-t pt-6">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-x-2 md:gap-y-4">
    ...
    <div class="vehicle-detail-mobile-cta__actions{{ $contactPhoneAvailable ? '' : ' vehicle-detail-mobile-cta__actions--single' }}">
        @if($contactPhoneAvailable)
            ...
            >
                ...
                <span>{{ __('messages.pages.vehicles.detail.call_seller') }}</span>
            </button>
            @endif
        @endif
        <button
