...
@section('title', ($branding['dealer_name'] ?? '') . ' — ' . __('messages.pages.inventory_audit.title'))

@section('content')
<style>
    :root {
        --audit-primary: {{ $branding['primary_color'] ?? '#2563eb' }};
        --audit-secondary: {{ $branding['secondary_color'] ?? '#1e40af' }};
    }
    .audit-branded-btn {
        background-color: var(--audit-primary);
        color: #fff;
    }
    .audit-branded-btn:hover {
        opacity: 0.92;
    }
    .audit-branded-header {
        border-bottom: 3px solid var(--audit-primary);
    }
</style>
<div class="container mx-auto px-4 py-10 max-w-3xl">
    <div class="audit-branded-header flex items-center gap-4 mb-8 pb-4">
        @if (!empty($branding['logo_url']))
            <img src="{{ $branding['logo_url'] }}" alt="{{ $branding['dealer_name'] }}" class="h-12 w-auto object-contain" />
        @endif
        <div>
            <p class="text-sm text-muted-foreground mb-1">{{ $branding['dealer_name'] }}</p>
            <h1 class="text-3xl font-bold mb-1">{{ __('messages.pages.inventory_audit.title') }}</h1>
            <p class="text-muted-foreground">{{ __('messages.pages.inventory_audit.subtitle') }}</p>
        </div>
    </div>
    ...
    <div id="audit-form" class="space-y-4 mb-8">
        ...
        @endfor
        <button type="button" id="run-audit" class="audit-branded-btn px-6 py-2 rounded font-medium">
