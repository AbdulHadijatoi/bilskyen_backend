...
<div class="flex h-full w-full flex-col items-center justify-center gap-4">
    <div class="flex w-full flex-col space-y-2">
        <h1 class="text-2xl font-semibold tracking-tight">
            {{ __('messages.pages.magic_link.login.title') }}
        </h1>
        <p class="text-sm text-muted-foreground">
            {{ __('messages.pages.magic_link.login.description') }}
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('magic-link.login.post') }}" class="grid w-full gap-3.5">
        @csrf
        ...
        <button type="submit" class="inline-flex h-10 w-full items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50">
            {{ __('messages.pages.magic_link.login.send_button') }}
        </button>
    </form>

    <div class="mt-4 text-center text-sm">
        {{ __('messages.pages.magic_link.login.already_have_account') }} <a href="/auth/login" class="underline">{{ __('messages.pages.magic_link.login.login_with_password') }}</a>
    </div>
</div>
@endsection
