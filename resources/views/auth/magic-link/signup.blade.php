...
@section('title', __('messages.pages.signup.magic_link_signup') . ' - Bilskyen')

@section('content')
<div class="flex h-full w-full flex-col items-center justify-center gap-4">
    <div class="flex w-full flex-col space-y-2">
        <h1 class="text-2xl font-semibold tracking-tight">
            {{ __('messages.pages.magic_link.signup.title') }}
        </h1>
        <p class="text-sm text-muted-foreground">
            {{ __('messages.pages.magic_link.signup.description') }}
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('magic-link.signup.post') }}" class="grid w-full gap-3.5">
        @csrf
        <div class="grid gap-2">
            ...
            <input id="name" name="name" type="text" placeholder="{{ __('messages.forms.placeholders.full_name') }}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
        </div>
        ...
        <div class="grid gap-2">
            ...
            <input id="email" name="email" type="email" placeholder="{{ __('messages.forms.placeholders.email') }}" autocomplete="email" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
        </div>
        ...
        <button type="submit" class="inline-flex h-10 w-full items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50">
            {{ __('messages.pages.magic_link.signup.send_button') }}
        </button>
    </form>
