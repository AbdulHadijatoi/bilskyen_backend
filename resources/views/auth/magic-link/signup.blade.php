...
<div class="flex h-full w-full flex-col items-center justify-center gap-4">
    <div class="flex w-full flex-col space-y-2">
        ...
        <p class="text-sm text-muted-foreground">
            Enter your information and we'll send you a magic link to create your account.
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('magic-link.signup.post') }}" class="grid w-full gap-3.5">
        ...
        <div class="grid gap-2">
            ...
            <input id="email" name="email" type="email" placeholder="johndoe@mail.com" autocomplete="email" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
        </div>
